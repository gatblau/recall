...
Feature: Tracing
  ...
  Scenario: Edge case — async extraction links to the triggering request
    Given a remember enqueues an ExtractFact job
    When the Write worker processes it
    Then its span shares the trace id of the original "api.remember" span
